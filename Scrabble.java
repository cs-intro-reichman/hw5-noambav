...
public class Scrabble {
	...
	static final Map<Character, Integer> SCRABBLE_LETTER_MAP = createScrabbleAlphabetMap(ALPHABET,
			SCRABBLE_LETTER_VALUES);

	static private Map<Character, Integer> createScrabbleAlphabetMap(char[] alphabet, int[] letterValues) {
		Map<Character, Integer> map = new HashMap<>();
		for (int i = 0; i < alphabet.length; i++)
			map.put(alphabet[i], letterValues[i]);
	...
	public static int wordScore(String word) {
		...
		for (int i = 0; i < word.length(); i++) {
			score += SCRABBLE_LETTER_MAP.get(word.charAt(i));
		}
		score *= word.length();
		...
		while (n > 0) {
			...
			if (MyString.subsetOf(input, hand))
				if (isWordInDictionary(input)) {
					int wordWorth = wordScore(input);
					score += wordWorth;
					System.out.println(input + " earned " + wordWorth + " points. Score: " + score + "points\n");
	...
	public static void playGame() {
		...
		while (true) {
			...
			if (input.equals("n")) {
				...
				System.out.println("Error... (You must choose between 'n' or 'e' as input)");
		}
	}
